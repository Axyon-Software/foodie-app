                        ...
                        <div className="px-6 pb-8">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h2 className="text-lg font-bold" style={{ color: 'var(--color-text)' }}>Cancelar pedido</h2>
                                </div>
                                <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full" style={{ backgroundColor: 'var(--color-bg-secondary)' }}>
                                    <X size={16} style={{ color: 'var(--color-text-secondary)' }} />
                                </button>
                            </div>
                            <p className="text-sm mb-6" style={{ color: 'var(--color-text-secondary)' }}>Tem certeza que deseja cancelar este pedido?</p>
                            <div className="space-y-2 mb-4">
                                {CANCEL_REASONS_CLIENT.map((reason) => (
                                    <button
                                        ...
                                        style={{
                                            backgroundColor: selectedReason === reason ? 'rgba(0, 160, 130, 0.1)' : 'var(--color-bg-secondary)',
                                            borderWidth: '1px', borderStyle: 'solid',
                                            borderColor: selectedReason === reason ? '#00A082' : 'transparent',
                                            color: 'var(--color-text)',
                                        }}
                                    >
                                        <div className="flex h-5 w-5 items-center justify-center rounded-full border-2" style={{ borderColor: selectedReason === reason ? '#00A082' : 'var(--color-border)' }}>
                                            {selectedReason === reason && <div className="h-2.5 w-2.5 rounded-full bg-[#00A082]" />}
                                        </div>
                                        {reason}
                                    </button>
                                ))}
                            ...
                            <div className="flex gap-3 mt-6">
                                <button onClick={onClose} className="flex-1 rounded-xl py-3 text-sm font-semibold" style={{ backgroundColor: 'var(--color-bg-secondary)', color: 'var(--color-text)' }}>Voltar</button>
                                <button onClick={handleCancel} disabled={isSubmitting || !selectedReason}
                                    className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-[#FF4444] py-3 text-sm font-semibold text-white hover:bg-[#E03E3E] disabled:opacity-50">
                                    {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : 'Confirmar cancelamento'}
                                </button>
                            </div>
                        </div>
                    </motion.div>
